...
bool sendSMS(const String &number, const String &message) {
  ...
  bool ok = res.indexOf("+CMGS:") != -1 && res.indexOf("ERROR") == -1;
  if (!ok) {
    // Both empty response and explicit ERROR (e.g. CMS ERROR: unknown subscriber)
    // mean the network rejected this specific number — the modem itself is fine.
    // Do NOT mark modemReady false here. dispatchMessage will continue to the
    // next phone number in the row.
    Serial.println("[SMS] Delivery failed for this number — modem stays ready");
  }
  return ok;
}
...
static int16_t dispatchMessage(size_t messageIndex) {
  ...
  for (size_t i = 0; i < PHONE_SLOTS_PER_MESSAGE; ++i) {
    ...
    // If a previous send marked modem not ready, attempt reinit before
    // trying the next number — a single bad number shouldn't block the rest
    if (!modemReady) {
      Serial.println("[MODEM] Reinitialising before next number...");
      initModem();
      if (!modemReady) {
        Serial.printf("[SMS] Skipping number %u — modem reinit failed\n", (unsigned)i);
        continue;
      }
    }
